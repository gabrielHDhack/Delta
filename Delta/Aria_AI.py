...
import pyautogui
import webbrowser
from STOP import main2 
from Tictactoe2 import main
from patterns_and_responses import propt, reminder_phrases, speech_off, name_change_keywords
import random 
import pywhatkit
import subprocess
    ...
    def run(self):
        ...
        while True:
            ...
            if command:
                ...
                if 'close the program' in command or 'end the program' in command or 'stop the program' in command or 'shutdown the program' in command or 'quit the program' in command or 'sleep' in command or 'shut down' in command:
                ...
                elif any(keyword in command for keyword in speech_off):
                    self.speak_response("I will enter in the silence mode")
...
#listen function
    @staticmethod
    def listen_for_command(timeout_seconds=5):
        try:
            while True:  # Continue listening until a command is recognized
                with sr.Microphone() as source:
                    ...
                    if 'aria' in command:
                        return command  # Return the command if "delta" is detected
                    else:
                        print("Name 'Aria' not detected. Listening again...")                   
        except sr.WaitTimeoutError:
            print(f"Speech recognition timeout ({timeout_seconds} seconds)")
            return None
        except sr.UnknownValueError:
    ...
    def generate_response(self, command):
        ...
        current_temperature_C = data['main']['temp'] - 273.15

        # Convertendo para Fahrenheit
        current_temperature_F = round((current_temperature_C * 9/5) + 32, 1)

        weather_description = data['weather'][0]['description']
        weather_message = f"The approximate temperature is {current_temperature_F} degrees Fahrenheit and the weather is {weather_description}"

        prompt = f"{propt} {user_name}:\n{context} {formatted_time} {weather_message}"
...
class VirtualAssistant(QWidget):
    def __init__(self):
        ...
        self.setWindowIcon(QIcon(r"C:\Delta\UI5.jpg"))

        # Tornando a janela um pouco transparente
        self.setWindowOpacity(0.7)
        self.setWindowFlag(Qt.WindowStaysOnTopHint)

        # Ajustando a folha de estilo para uma cor mais transparente
        ...
        self.message_label.setStyleSheet(
            "background-color: rgba(0, 0, 0, 150); color: #FFFFFF; margin-bottom: 20px; padding: 10px; border-radius: 5px;")

        self.gif_label = QLabel(self)
        self.gif_label.setGeometry(10, 10, 100, 100)
        self.load_gif(r"C:\Delta\gif5.gif")
        ...
        self.button_listen_continuous.setStyleSheet(
            "background-color: rgba(0, 0, 0, 150); color: #FFFFFF; padding: 10px 20px; font-size: 14px; border: none; border-radius: 5px;")
    ...
    def lock_screen(self):
        ...
        self.command_processor.speak("Screen locked.")
    ...
    def listen_for_name(self):
        try:
            ...
            messages = f"Hi {name}, nice to meet you! If your name is incorrect, feel free to let me know so I can change it by saying the sentence 'change my name'."
            self.command_processor.speak(messages)
            self.update_message(messages)
            time.sleep(1)
            self.is_listening_continuous = True
